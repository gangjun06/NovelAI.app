...
export interface SwitchProps
  extends Omit<React.PropsWithoutRef<JSX.IntrinsicElements['button']>, 'onChange' | 'value'> {
  label: string
  checked: boolean
    ...
    return (
      <UISwitch.Group>
        <div className={classNames('flex items-center gap-3')}>
          <UISwitch.Label className="text-sm text-base-color">{label}</UISwitch.Label>
          <UISwitch
            {...props}
            name={name}
            ...
            className={classNames(
              checked ? 'bg-primary-400 dark:bg-primary600' : 'bg-primary-200 dark:bg-primary-800',
