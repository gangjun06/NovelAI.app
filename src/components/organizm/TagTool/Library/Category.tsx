    ...
    useCallback(
      (get, _) => {
        ...
        category.tags.forEach((tagAtom) => {
          ...
          result += `${replaceText(
            formatPriority(tag.tag, tag.priority, priorityChar[0], priorityChar[1]),
            withUnderbar,
          )}, `
        })
  ...
  return (
    <div
      className={classNames(
        ...
        over && over.data.current?.sortable?.containerId === `category-${categoryAtom}`
    ...
    >
      <div className="flex gap-x-2 items-center">
        {rename !== null ? (
          ...
          <button
            onClick={() => setCategory((prev) => ({ ...prev, isOpen: !prev.isOpen }))}
      ...
      <SortableContext
        items={[...category.tags.map((tagAtom) => `${tagAtom}`), PLACEHOLDER_ID]}
        id={`category-${categoryAtom}`}
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        //@ts-ignore
        strategy={() => {}}
      >
        {category.isOpen && !((active?.id ?? '') as string).startsWith('container-') && (
          <div className="pb-2">
    ...
    useCallback(
      (get, set, index: number) => {
        set(categoryAtom, ({ tags, ...prev }) => ({
          ...prev,
          tags: [...tags.slice(0, index + 1), atom(get(tags[index])), ...tags.slice(index + 1)],
        }))
      },
      [categoryAtom],
    ),
      ...
      {!category.tags.length && (
        <>
          <div className="text-description-color flex gap-x-1">
            <CheckIcon className="w-5 h-5" /> 아이콘 활성화 후 왼쪽에서 클릭하여 추가하세요!
          </div>
          <TagToolPlaceholder />
        </>
      )}
